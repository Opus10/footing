...
def up_to_date(version=None):
    ...
    new_template_version = version or _get_latest_template_version(
        footing_config['_template']
    )

    return new_template_version == old_template_version


def _needs_new_cc_config_for_update(
    ...
    if old_template != new_template:
        ...
        return _cookiecutter_configs_have_changed(
            new_template, old_version, new_version
        )
...
def update(
    ...
    footing.utils.shell(
        'git checkout -b {}'.format(update_branch), stderr=subprocess.DEVNULL
    )
    ...
    _apply_template(
        old_template, '.', checkout=old_version, extra_context=footing_config
    )
    ...
    footing.utils.shell(
        'git checkout {}'.format(update_branch), stderr=subprocess.DEVNULL
    )
    footing.utils.shell(
        'git merge -s ours --no-edit --allow-unrelated-histories {}'.format(
            temp_update_branch
        ),
        stderr=subprocess.DEVNULL,
    )
    ...
    footing.utils.shell(
        'git checkout {}'.format(temp_update_branch), stderr=subprocess.DEVNULL
    )
    footing.utils.shell('git rm -rf .', stdout=subprocess.DEVNULL)
    ...
    _apply_template(
        new_template, '.', checkout=new_version, extra_context=footing_config
    )
    footing.utils.write_footing_config(
        footing_config, new_template, new_version
    )
